...
import copy
import h5py
import src.landscape_utils.net_plotter as net_plotter
# import src.landscape_utils.h5_util as h5_util
from src.model import CNNModel
from src.landscape_utils import h5_util
...
def project_trajectory(args, dir_file, w, s, dataset, model_name, model_files,
    ...
    for model_file in model_files:
        net2 = CNNModel(model=args.model, classes=args.num_classes, pretrained=False)
        net2.load_state_dict(torch.load(os.path.join(args.weight_path, args.epoch+'.pth')))
        if dir_type == 'weights':
